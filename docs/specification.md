...
The PTC cluster consists of 1 **master node** and 26 **compute nodes** now. There is no login server and the SSH connection to `ptc.ibs.re.kr` directs to the master node. All the machines are in the data center located on the ground floor of the experiment building in the IBS main site.
...
Each compute node has quite a number of CPU cores, 24 to 72 cores with clock speeds of 2.1 GHz to 2.6 GHz. The total accumulated number of CPU cores of all the compute node is 1216. The hyper-threading of CPU is disabled for all nodes. Each node has more than 128 GB memory.
